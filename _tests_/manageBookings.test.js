const {
  ...
  describe('Booking Management', () => {
    ...
    test('fetchBookings should fetch bookings from API and render them', async () => {
      document.body.innerHTML = '<div id="bookingsContainer"></div>'; // Mock the bookings container
  
      await fetchBookings();
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('bookings'), expect.anything());
      expect(bookings).toEqual(mockBookings);
      // Ensure renderBookings is called by verifying DOM content
    ...
    test('renderBookings should render filtered bookings', () => {
      document.body.innerHTML = `
        <div id="bookingsContainer"></div>
        <select id="statusFilter"><option value="">All</option><option value="confirmed">Confirmed</option></select>
        <select id="roomFilter"><option value="">All</option><option value="Hall">Hall</option></select>
        <input id="searchInput" value="One" />
      `;
      renderBookings();
      
      const container = document.getElementById('bookingsContainer');
      expect(container.children.length).toBe(1); // Only one booking should match the search query
    });
